...
public class F_KeyboardInput {
	...
	public void inputScanner1() {
		...
		Scanner sc = new Scanner(System.in); // -> 입력받은 값을 byte 단위로 받아들이겠다.

		System.out.println("당신의 이름은 무엇입니까?");

		// 사용자가 입력한 값을 문자열로 받아오는 메소드(next(), nextLine())
		// String name = sc.next(); // 공백이 있는 경우 공백 이전까지 읽어 옴. 즉, 띄어쓰기 전까지
		String name = sc.nextLine(); // 입력한 값 모두 읽어 옴

		System.out.println("당신의 나이는 몇 살입니까?");

		int age = sc.nextInt(); // 입력한 정수에 대한 값만 읽어 옴

		System.out.println("당신의 키는 몇입니까?");

		double height = sc.nextDouble(); // 입력한 실수에 대한 값만 읽어 옴
	...
	public void inputScanner3() {
		...
		//sc.nextChar() -> 존재하지 않음.
		//CharAt : 문자열에서 지정된 위치에 있는 문자 하나만 뽑아내는 기능을 함. 이때 위치는 제일 앞 문자가 0부터 시작해서 1씩 증가
