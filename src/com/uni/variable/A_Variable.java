...
public class A_Variable {
	// 변수란 : 어떠한 값을 메모리에 기억하기 위한 공간이다 (박스같은개념)
		...
		public void declareVariable() {
			...
			byte bNum;  // 1byte
			short sNum;  // 2byte
			int iNum;  // 4byte -> 정수형 중 가장 대표적인 자료형(기본형) -약21억~21억 값이 담기는 자료형
			long lNum;  //8byte
			...
			dNum = 8.0;  // d 붙이지 않아도 됨
			
			ch = 'A';  // 따옴표 사용해서 값 대입
			// ch = 'ABC';  // 문자에는 한 문자 값이 저장됨 - 에러
			ch = '강';  // 문자에 한글도 들어감
			
			str = "ABC";  // 쌍따옴표 사용해서 값 대입
			str = "A";  // 한 개의 문자만 쓰더라도 쌍따옴표!!
		...
		public void initVariable() {
			...
			short sNum = 2;
			int iNum = 5;
			long lNum = 8L;  // L 붙여야 함
			
			float fNum = 4.0f;  // 반드시 f 붙여야 함
			double dNum = 8.0;  // d 붙이지 않아도 됨
			
			char ch = 'A';  // 따옴표 사용해서 값 대입
			// ch = 'ABC';  // 문자에는 한 문자 값이 저장됨 - 에러
			char ch1 = '강';  // 문자에 한글도 들어감
			//꼭 ch에 담아야 되면 ch = '강'; 자료혀여 없이 대입하면 됨
			
			String str = "ABC";  // 쌍따옴표 사용해서 값 대입
			String str1 = "A";  // 한 개의 문자만 쓰더라도 쌍따옴표!!
			...
			//4) 숫자 가능 (단, 숫자로 시작은 안 됨)
			//int 1age;
			
			//5) 특수문자 가능 (단, _ $ 이외의 특수문자 불가능)
			int number_1;
			int number$2;
			//int number#1;
			
			//6) 여러 단어로 된 변수명일 경우, 붙여쓰고 연결되는 단어의 첫 글자 대문자
			String username;  // 관례상 틀림
			String userName;  // 관례상 맞음
			
			String 이름;  // 한글 잘 사용하지 않음
		...
		public void test() {
			...
			System.out.println("v1값 " + v1);
			System.out.println("v2값 " + v2);
			System.out.println("v3값 " + v3);
			System.out.println("v4값 " + v4);
			System.out.println("v5값 " + v5);
