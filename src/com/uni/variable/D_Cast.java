...
public class D_Cast {
	...
	public void rule1() {
		...
		/*
		 * 자동형변환 : 값의 범위가 작은 자료형과 큰 자료형 간의 연산(대입, 계산) 시 컴파일러가 자동으로 범위가 작은 자료형을 큰 자료형으로 변환
		 * 
		 */

		short s = 1;
		int i = s; // 분명 대입 연산자(=)를 기준으로 왼오 자료형이 다르지만 문제 없음
		...
		// 4. long(8byte) -> flaot(4byte) -> 크기는 long 형이 더 크지만 float 형으로 자동형변환이 가능한 이유는
		                                  // 표현 가능한 수의 범위가 더 크기 때문
		...
		int result3 = b2 + b3; // byte 나 short 연산 시 무조건 int 로 처리. 따라서 강제 형변환 필요
		byte result4 = (byte) (b2 + b3);

		/*
		 * int형 리터럴로 초기화가 가능한 자료형 byte, short, char, int -> 1) 초기화 시 입력 값을 int형 리터럴로 인식하지 않고 그냥 정수로 인식 
		 * 2) 컴파일러가 저장하려는 자료형의 값의 범위와 일치하는지 검사하여 일치하면 입력값을 저장하려는 자료형으로 변환하여 저장
	...
	public void rule2() {
		...
		// int iSum = iNum + dNum; //10.0 + 5.89 -> 15.89 double형 -> int 형 변수로 대입 불가능

		// 해결방법
		// 1. 수행 결과를 int형으로 강제 형변환
		int iSum = (int) (iNum + dNum); // 실수 > 정수 : 강제 형변환 발생 시 소수점 이하 모두 버려짐

		System.out.println("iSum " + iSum);

		// 2. double 형 값을 int 형으로 강제 형변환
		int iSum2 = iNum + (int) dNum;
		System.out.println("iSum2 " + iSum2);

		// 3. 연산 결과를 double 형으로 받음
		...
		System.out.println(temp2); // 계산하기 쉽지 않은 데이터 손실이 발생하기 때문에 주의해서 사용

	}

	public void stringTest() {

		// 리터럴 : stringpool 에서는 이미 값이 있으면 주소 생성하지 않음
		// new : 무조건 heap에 새로운 객체 생성, 반환 타입이 참조형 이라는 것은 객체의 주소를 반환한다는 것을 의미

		String s = "안녕하세요"; // heap(stringpool)
		String s1 = "안녕하세요"; // heap(stringpool) 이미 존재
		String s2 = new String("안녕하세요"); // heap 안에 새로 생성
		...
		String t2 = new String("유재석"); // heap 에 생성
		...
		/*
		 * HashCode : 객체가 메모리에 가진 해쉬 주소값을 의미 (Java)에서 가지고 있는 주소값 System.identityHashCode
		 * : 특별한 설정을 하지않은경우 hashcode와 같이 주소값을 의미 ,os(system)에서 가지고 있는 주소값
		 */
